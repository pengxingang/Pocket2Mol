...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='./configs/sample/vn_sample.yml')
    parser.add_argument('--outdir', type=str, default='./outputs')
    parser.add_argument('--device', type=str, default='cuda')
    parser.add_argument('--data_id', type=str, default='41')
    args = parser.parse_args()

        # check exist in out dir
    all_files = os.listdir(args.outdir)
    file_name = os.path.basename(args.config)[:-4] + '_' + args.data_id + '_'
    # file_name = 'vn_sample_' + args.data_id + '_'
    ...
    # model.config.update({'knn_q_cps': })config.model.field.knn
    ...
    if config.sample.mask_init:
        ...
        init_data_list = get_init(data.to(args.device), 
                model = model,
